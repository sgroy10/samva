...
{{
    ...
    # Build PDF — use write() for all text to avoid multi_cell width issues
    pdf = FPDF()
    ...
    pdf.set_font("Helvetica", "B", 18)
    pdf.write(10, "Travel Itinerary\n")
    pdf.set_font("Helvetica", "", 11)
    pdf.write(7, _safe_text(f"{dest} | {duration} | For {user_name or 'You'}\n"))
    pdf.ln(5)

    for day_info in itinerary_data.get("days", []):
        pdf.set_font("Helvetica", "B", 13)
        pdf.write(8, _safe_text(f"Day {day_info.get('day', '')} - {day_info.get('title', '')}\n"))
        pdf.set_font("Helvetica", "", 10)

        for slot in ["morning", "lunch", "afternoon", "evening", "dinner"]:
            val = day_info.get(slot, "")
            if val:
                pdf.write(5, _safe_text(f"  {slot.title()}: {val}\n"))

        tips = day_info.get("tips", "")
        if tips:
            pdf.set_font("Helvetica", "I", 9)
            pdf.write(5, _safe_text(f"  Tip: {tips}\n"))
        pdf.ln(3)

    budget = itinerary_data.get("budget_estimate", "")
    if budget:
        pdf.set_font("Helvetica", "B", 11)
        pdf.write(7, _safe_text(f"Budget: {budget}\n"))
    ...
    if notes:
        ...
        pdf.set_font("Helvetica", "", 10)
        for note in notes:
            pdf.write(5, _safe_text(f"  - {note}\n"))

    pdf.ln(4)
    pdf.set_font("Helvetica", "I", 8)
    pdf.write(5, f"Generated by Sam | {now.strftime('%d %b %Y %I:%M %p IST')}")
