...
{{
    ...
    # Build PDF — simple layout, width=0 means full page
    pdf = FPDF()
    ...
    pdf.set_font("Helvetica", "B", 18)
    pdf.cell(0, 10, "Travel Itinerary", new_x="LMARGIN", new_y="NEXT", align="C")
    pdf.set_font("Helvetica", "", 11)
    pdf.cell(0, 7, _safe_text(f"{dest} | {duration} | For {user_name or 'You'}"), new_x="LMARGIN", new_y="NEXT", align="C")
    pdf.ln(5)

    for day_info in itinerary_data.get("days", []):
        pdf.set_font("Helvetica", "B", 13)
        pdf.cell(0, 8, _safe_text(f"Day {day_info.get('day', '')} - {day_info.get('title', '')}"), new_x="LMARGIN", new_y="NEXT")
        pdf.set_font("Helvetica", "", 10)

        for slot in ["morning", "lunch", "afternoon", "evening", "dinner"]:
            val = day_info.get(slot, "")
            if val:
                pdf.multi_cell(0, 5, _safe_text(f"  {slot.title()}: {val}"))

        tips = day_info.get("tips", "")
        if tips:
            pdf.set_font("Helvetica", "I", 9)
            pdf.multi_cell(0, 5, _safe_text(f"  Tip: {tips}"))
        pdf.ln(3)

    budget = itinerary_data.get("budget_estimate", "")
    if budget:
        pdf.set_font("Helvetica", "B", 11)
        pdf.cell(0, 7, _safe_text(f"Budget: {budget}"), new_x="LMARGIN", new_y="NEXT")
    ...
    if notes:
        ...
        pdf.set_font("Helvetica", "", 10)
        for note in notes:
            pdf.multi_cell(0, 5, _safe_text(f"  - {note}"))

    pdf.ln(4)
    pdf.set_font("Helvetica", "I", 8)
    pdf.cell(0, 5, f"Generated by Sam | {now.strftime('%d %b %Y %I:%M %p IST')}")
...
User's name: {user_name}""",
    ...
    logger.info(f"[{user_id}] Custom PDF content: title={content.get('title')}, sections={len(content.get('sections',[]))}")
    ...
    try:
        ...
        # Use simple text writing — no width calculations
        ...
        logger.info(f"[{user_id}] Custom PDF: {len(pdf_bytes)}B, has_text={b'BT' in pdf_bytes}")
        b64 = base64.b64encode(pdf_bytes).decode()
        return (b64, f"{title} - {now.strftime('%d %b %Y')}")
    except Exception as e:
        logger.error(f"[{user_id}] Custom PDF build failed: {e}", exc_info=True)
        return ("", "")
