...
User's name: {user_name}""",
    ...
    for i, s in enumerate(content.get("sections", [])):
        logger.info(f"[{user_id}] Section {i}: heading={s.get('heading','')[:30]}, content_len={len(s.get('content',''))}")
    ...
    try:
        ...
        for section in content.get("sections", []):
            ...
            if heading:
                pdf.set_font("Helvetica", "B", 12)
                pdf.cell(pw, 8, heading, ln=True)
            pdf.set_font("Helvetica", "", 10)
            for line in body.split("\n"):
                cleaned = _safe_text(line.strip())
                if cleaned:
                    pdf.set_x(pdf.l_margin)
                    pdf.multi_cell(pw, 5, cleaned)
            pdf.ln(3)
        ...
        logger.info(f"[{user_id}] Custom PDF generated: {len(pdf_bytes)} bytes, has_BT={b'BT' in pdf_bytes}")
