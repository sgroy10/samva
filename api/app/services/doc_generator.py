...
{{
    ...
    pdf.cell(0, 8, _safe_text(f"{dest} | {duration} | For {user_name or 'You'}"), ln=True, align="C")
    pdf.ln(10)
    pdf.set_text_color(0, 0, 0)
    ...
    for day_info in itinerary_data.get("days", []):
        ...
        pdf.cell(0, 10, _safe_text(f"Day {day_info.get('day', '')} - {day_info.get('title', '')}"), ln=True, fill=True)
        pdf.set_font("Helvetica", "", 10)
        pdf.ln(2)
    ...
    if budget:
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, _safe_text(f"Estimated Budget: {budget}"), ln=True)
    ...
    if notes:
        ...
        pdf.set_font("Helvetica", "", 10)
        for note in notes:
            pdf.multi_cell(0, 6, _safe_text(f"  - {note}"))
...
User's name: {user_name}""",
    ...
    for section in content.get("sections", []):
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, _safe_text(section.get("heading", "")), ln=True)
        pdf.set_font("Helvetica", "", 10)
        for line in section.get("content", "").split("\n"):
            if line.strip():
                pdf.multi_cell(0, 6, _safe_text(line.strip()))
        pdf.ln(4)
